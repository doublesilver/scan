...
- Status: [x] 완료
- 완료 조건:
  - [x] APK v5.3.12 설치 (Mini PC C:\scanner\)
  - [x] DataWedge 프로파일 자동 생성 확인
  - [x] 실제 바코드 스캔 테스트 (0.3~0.5초 목표)
  - [x] 장바구니 + 라벨 인쇄 워크플로우 E2E
  - [x] 오프라인 모드 동작 확인
  - [x] 현장 WiFi 환경 테스트

## M15: 도면 API 책임 분리 + 에디터 고도화 (v5.3.12)

- Status: [x] 완료
- 완료 조건:
  - [x] 도면 데이터 보호 (상품 있는 셀 삭제 차단)
  - [x] PATCH levels UPSERT (PDA 연결 상품 보존)
  - [x] 단일 소스 확정 (map_layout JSON 이중 저장 제거)
  - [x] 웹 에디터 기능 8개 추가 (다크모드, 페인트, 다중선택, 접기, 좌표, 툴팁, 자동저장, 통계)
  - [x] 셀 크기 조절, 텍스트 삽입, 테두리 스타일
  - [x] editor-meta 서버 영속 API
  - [x] 전수검사 Critical+High 5건 수정, 64 tests passed
