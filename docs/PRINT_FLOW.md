...
**조건:** `settings.print_agent_url`이 설정돼 있음

**동작:**

1. agent URL로 `POST` 전송: `{ "barcode": ..., "quantity": ..., "printer_name": ... }`
2. BarTender agent가 **바코드 끝 5자리**로 대응하는 `.btw` 템플릿 파일을 자동 매칭
3. 템플릿에 바코드·수량 주입 후 프린터로 출력
4. agent의 응답을 그대로 앱에 반환
...
- Mini PC에 BarTender print agent 서비스가 항상 돌고 있어야 함
...
**Mini PC(Windows)** 환경에 BarTender print agent 서비스가 상시 동작하고 있고, `.btw` 템플릿 파일들이 준비돼 있어서 **경로 A**로 인쇄가 이루어집니다. 바코드 끝 5자리 자동 매칭은 `73cbb13` 커밋에서 연결됐습니다.
...
1. Mini PC에서 BarTender print agent 서비스 상태 확인
2. `settings.print_agent_url` 설정 값이 올바른지 확인
3. 서버 로그에서 `print agent 호출 실패` 문구 검색
