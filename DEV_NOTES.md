...
- **우리가 받은 파일**: SKU ID 컬럼 1개만 존재 (잘린 파일로 추정)
- **실제 원본** (스크린샷 기반): 17개 컬럼
  - SKU ID, 업종, 상품코드, 바코드, 등가/비등가상태, 발주담당자, 발주담당자연락처, 정우식(?), 수량, 금액 등
- **서버 파서 구현 시**: 원본 xlsx 기준으로 설계해야 함
...
| sku_download 원본 xlsx (17컬럼) | 우리가 받은 건 1컬럼만 | **클라이언트에게 재요청** |
